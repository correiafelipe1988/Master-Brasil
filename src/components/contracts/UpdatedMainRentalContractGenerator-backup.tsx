...
VALOR TOTAL: {{total_amount}}`,
      ...
      // Cláusula 2: Anexo II - Introdução e Cláusulas 1-6
      const clause2 = await ContractTemplateService.createClause({
        template_id: templateId,
        clause_number: '2',
        title: 'ANEXO II - MINUTA DE CONTRATO (Parte 1)',
...
6.9- Caso o Locatário ultrapasse o limite diário de quilometragem, arcará com multa apontada por KM excedente, descrito no Anexo próprio denominado Tarifário Locagora;`,
      ...
      // Cláusula 3: Anexo II - Cláusulas 7-15
      const clause3 = await ContractTemplateService.createClause({
        template_id: templateId,
        clause_number: '3',
        title: 'ANEXO II - MINUTA DE CONTRATO (Parte 2)',
...
15.1- A LOCADORA poderá, a seu exclusivo critério, reajustar os valores relativos à diária, às tarifas, às taxas e outros incidentes sobre a locação, hipótese em que serão aplicados os valores vigentes no momento da prorrogação da locação pelo LOCATÁRIO, de acordo com o tarifário nacional vigente.`,
      ...
      // Cláusula 4: Anexo II - Cláusulas 16-21
      const clause4 = await ContractTemplateService.createClause({
        template_id: templateId,
        clause_number: '4',
        title: 'ANEXO II - MINUTA DE CONTRATO (Parte 3)',
        content: `CLÁUSULA 16ª- TRATAMENTO DE DADOS PESSOAIS
...
Testemunha 2`,
      ...
      console.log('✅ [MainRentalContract] Cláusulas atualizadas com dados do SILVIO ROBERTO criadas com sucesso');
  ...
  // Restante do código permanece igual ao MainRentalContractGenerator original...
  const getOrCreateMainContractTemplateId = async (): Promise<string> => {
    try {
      // Usar nome único com timestamp para forçar recriação
      const templateName = `Contrato Principal de Locação - SILVIO ROBERTO v${Date.now()}`;
      
      console.log('📋 [MainRentalContract] Criando novo template com conteúdo completo do SILVIO ROBERTO...');
      ...
      const newTemplate = await ContractTemplateService.createTemplate({
        ...
        name: templateName,
        version: '2.0',
        title: 'Instrumento Particular de Contrato de Locação de Motocicleta(s) com Possibilidade de Conversão em Cessão de Bem Móvel',
        content: {
          sections: [
            {
              title: "INSTRUMENTO PARTICULAR DE CONTRATO DE LOCAÇÃO DE MOTOCICLETA(S) COM",
              subtitle: "POSSIBILIDADE DE CONVERSÃO EM CESSÃO DE BEM MÓVEL"
            }
          ]
        },
        variables: [
      ...
      // Verificar se as cláusulas foram criadas corretamente
      const createdClauses = await ContractTemplateService.getTemplateClauses(newTemplate.id);
      console.log('✅ [MainRentalContract] Template criado com', createdClauses.length, 'cláusulas');
      
      console.log('✅ [MainRentalContract] Template baseado no SILVIO ROBERTO criado:', newTemplate);
  ...
  const loadAllContracts = async () => {
    if (rentalId) {
      try {
        // Buscar contratos que comecem com este nome base
  ...
  // Restante dos métodos idênticos ao original...
  const generateMainContract = async () => {
    ...
    try {
      ...
      const fileName = `contrato_principal_silvio_${generatedContract.contract_number}.pdf`;

      console.log('📄 [MainRentalContract] Contrato SILVIO ROBERTO gerado:', fileName);
      ...
      toast.success('Contrato Principal baseado no modelo SILVIO ROBERTO gerado com sucesso!');
      setIsDialogOpen(false);

      // Aguardar um momento para garantir que o contrato foi salvo no banco
      ...
      console.log('📋 [MainRentalContract] Recarregando contratos...');
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold">Contrato Principal (Modelo SILVIO ROBERTO)</h3>
        </div>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4">
              <Card>
                <CardHeader>
                  <CardTitle className="text-sm">Contrato de Locação - Modelo SILVIO ROBERTO</CardTitle>
                  <CardDescription>
                    Baseado no contrato #15239 com formato exato do arquivo fornecido
                  </CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <div className="flex gap-2">
                {existingContract ? (
                  ...
                  <Button
                    ...
                    className="flex-1"
                  >
                    {isGenerating ? 'Gerando...' : 'Gerar Contrato (Modelo SILVIO)'}
                  </Button>
                )}
              </div>
            </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-gray-600">
                      Modelo SILVIO ROBERTO - Criado em {new Date(contract.created_at).toLocaleDateString('pt-BR')}
                    </p>
                  </div>
